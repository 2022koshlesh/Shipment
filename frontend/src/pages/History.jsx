...
const ShipmentHistory = () => {
  ...
  const statusColors = {
    ...
    Cancelled:
      "bg-gradient-to-r from-error/10 to-error/20 text-error border border-error/20",
    Returned:
      "bg-gradient-to-r from-textSecondary/10 to-textSecondary/20 text-textSecondary border border-textSecondary/20",
  };
  ...
  const fetchHistory = async (currentFilters = filters) => {
    try {
      ...
      Object.entries(currentFilters).forEach(([key, value]) => {
        if (key === "status" && value === "") {
          cleanFilters[key] = "all"; // tell backend to return all
      ...
      let errorMessage =
        "Something went wrong while fetching your shipment history.";
  ...
  const exportToCSV = () => {
    ...
    a.setAttribute(
      "download",
      `shipment-history-${new Date().toISOString().split("T")[0]}.csv`
    );
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-surface to-background flex justify-center items-center">
        <div className="text-center bg-surface p-12 rounded-2xl shadow-card border border-border">
          ...
          <p className="text-textSecondary text-lg">
            Please log in to access your shipment history.
          </p>
        </div>
      </div>
    );
                ...
                <table className="min-w-full divide-y divide-border">
                  ...
                  <tbody className="bg-surface/50 divide-y divide-border">
                    {history.map((shipment) => (
                      <tr
                        key={shipment.id}
                        className="hover:bg-gradient-to-r hover:from-accent/5 hover:to-transparent transition-all duration-200"
                      >
                        ...
                        <td className="px-8 py-6 whitespace-nowrap">
                          <div className="text-sm text-textPrimary">
                            ...
                            <div className="text-accent text-center text-lg">
                              ↓
                            </div>
                        ...
                        <td className="px-8 py-6 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center px-3 py-1 rounded-xl text-xs font-semibold ${
                              ...
                              "bg-gradient-to-r from-textSecondary/10 to-textSecondary/5 text-textSecondary border border-textSecondary/20"
                            }`}
                          >
                            ...
                            <div className="text-xs text-textSecondary">
                              {shipment.package?.description
                                ? shipment.package.description.length > 30
                                  ? shipment.package.description.substring(
                                      0,
                                      30
                                    ) + "..."
                                  : shipment.package.description
                  ...
                  {[...Array(pagination.totalPages)].map((_, index) => {
                    ...
                    if (
                      ...
                      return (
                        <span
                          key={page}
                          className="px-2 py-2 text-sm text-textSecondary"
                        >
                          ...
                        </span>
                      );
